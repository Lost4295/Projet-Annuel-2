...
class AppartementService {
    ...
    public function updateService($servId)
    {
        $serv = $this->em->getRepository(Service::class)->find($servId);
        $no = $serv->getNotes();
        $notes = [];
        foreach ($no as $note) {
            $notes[] = $note->getNote();
        ...
        $serv->setAvgNote($average_rating);
        $this->em->persist($serv);
        $this->em->flush();
        return $output;
    }

    public function updateProfessionnel($servId)
    {
        $serv = $this->em->getRepository(Professionnel::class)->find($servId);
        $na = $serv->getServices();
        $notes = [];
        foreach ($na as $service){
            $snotes = $service->getNotes();
            foreach ($snotes as $note) {
                $notes[] = $note->getNote();
            }
        }
        $total_review = $total_user_rating = $five_star_review = $four_star_review
